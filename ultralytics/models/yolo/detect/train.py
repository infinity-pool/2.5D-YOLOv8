...
'''HWCHU. DetectionTrainer_2_5'''
class DetectionTrainer_2_5(BaseTrainer):
    """
    A class extending the BaseTrainer class for training based on a detection model.
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        """Return a YOLO detection model."""
        # model = DetectionModel(cfg, nc=self.data["nc"], verbose=verbose and RANK == -1)
        model = DetectionModel_2_5(cfg, nc=self.data["nc"], verbose=verbose and RANK == -1) # HWCHU. DetectionModel_2_5로 수정
        if weights:
            model.load(weights)
        return model
    ...
    def get_validator(self):
        """Returns a DetectionValidator for YOLO model validation."""
        # self.loss_names = "box_loss", "cls_loss", "dfl_loss"
        self.loss_names = "box_loss", "cls_loss", "dfl_loss", "dist_loss" # HWCHU. "dist_loss" 추가
        # return yolo.detect.DetectionValidator(
        #     self.test_loader, save_dir=self.save_dir, args=copy(self.args), _callbacks=self.callbacks
        # )
        return yolo.detect.DetectionValidator_2_5(
            self.test_loader, save_dir=self.save_dir, args=copy(self.args), _callbacks=self.callbacks
        ) # HWCHU. DetectionValidator_2_5
