...
class Results_2_5(SimpleClass):
    ...
    def __init__(
        ...
        self.orig_shape = orig_img.shape[:2]
        self.boxes = Boxes(boxes, self.orig_shape) if boxes is not None else None  # native size boxes
        self.dists = Dists(dists, self.orig_shape) if dists is not None else None # HWCHU. dists라는 property 추가
        self.masks = Masks(masks, self.orig_shape) if masks is not None else None  # native size or imgsz masks
        self.probs = Probs(probs) if probs is not None else None
        self.keypoints = Keypoints(keypoints, self.orig_shape) if keypoints is not None else None
        self.obb = OBB(obb, self.orig_shape) if obb is not None else None
    ...
    def plot(
        ...
        labels=True,
        boxes=True,
        dists=True, # HWCHU. dists를 보여줄건지 확인에 쓰임.(아래 pred_dists, show_dists)
        masks=True,
        probs=True,
        show=False,
        save=False,
        ...
        is_obb = self.obb is not None
        pred_boxes, show_boxes = self.obb if is_obb else self.boxes, boxes
        pred_dists, show_dists = self.dists, dists # HWCHU. 예측된 dists와 그를 보여줄건지 확인
        pred_masks, show_masks = self.masks, masks
        pred_probs, show_probs = self.probs, probs
        # HWCHU. i.e) pred_boxes.shape(5, 6), pred_dists.shape(5, 1)
        ...
        # Plot Segment results
        if pred_masks and show_masks: # HWCHU. 일단 안들어감.
            if im_gpu is None:
                img = LetterBox(pred_masks.shape[1:])(image=annotator.result())
                im_gpu = (
                    torch.as_tensor(img, dtype=torch.float16, device=pred_masks.data.device)
        ...
        # Plot Detect results
        if pred_boxes is not None and show_boxes: # HWCHU. 여기 확인.
            # for i, d in enumerate(reversed(pred_boxes)):
            for i, (d, d_dist) in enumerate(zip(reversed(pred_boxes), reversed(pred_dists))): # HWCHU. pred_dists도 함께 for문 돌아.
                c, conf, id = int(d.cls), float(d.conf) if conf else None, None if d.id is None else int(d.id.item())
...
'''HWCHU. Dists(BaseTensor). detect_2_5 수행 시, dist 정보를 담기 위한 class 신규 생성'''
class Dists(BaseTensor):
    def __init__(self, dists, orig_shape) -> None:
        """
        This class is implemented by HWCHU to describe distances to objects(boxes)
        """
        if dists.ndim == 1:
            dists = dists[None, :]
        super().__init__(dists, orig_shape)
        self.orig_shape = orig_shape
    
    @property
    def dist(self):
        """
        Returns the dists for each detection box.
        """
        return self.data[:, 0]
